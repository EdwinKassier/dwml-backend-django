...
class DataCache:
    ...
    def insert_into_logging(self):
        ...
        try:
            ...
            logging_item = LOGGING(SYMBOL=self.coin_symbol, INVESTMENT=self.investment,GENERATIONDATE=datetime.now().isoformat())
    ...
    def insert_into_result(self,result):
        ...
        try:
            result_item = Results(NUMBERCOINS=result['NUMBERCOINS'], PROFIT=result['PROFIT'] ,GROWTHFACTOR=result['GROWTHFACTOR'],LAMBOS=result['LAMBOS'],INVESTMENT=self.investment,SYMBOL=self.coin_symbol,GENERATIONDATE=datetime.now().isoformat())
